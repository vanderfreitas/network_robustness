...
print('ROBUSTNESS: FLOW - FAILURES')
...
for thresh in [0, avg, avg+std]:
	...
	# robustness
	number_removed,P_infty = robustness_flow_sum_F_failure_node(g_original, f_matrix, codes, simulations=20)

	# Save data to disk
	file = open(relative_path + 'robustness_flow_sum_F_failure_' + str(thresh) + '.csv', 'w')
	...
	file_out = open(relative_path + 'robustness_flow_sum_F_failure_R_V_' + str(thresh) + '.csv', 'w')
